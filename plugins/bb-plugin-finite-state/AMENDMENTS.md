...
- Artifacts:
  - `plugins/bb-plugin-finite-state/lib/store/schema.ts`
- Phase 2 contract artifact: `plugins/bb-plugin-finite-state/shared/contract.ts`; packaged by the zero-substance AMD-0023 sibling after the schema-only AMD-0021 accept merged.
- Contract version: 8
...
- New artifact hashes:
  ...
  - `shared/contract.ts`: `041dc7dec66ac92d33b48e947412a2eedff4e7e6bdb54122f5623c9aebd0c466` (accepted by the phase-2 AMD-0023 packaging record)
...
- Acceptance sequencing: the B1 schema target merged under AMD-0021 while `shared/contract.ts` remained at its AMD-0020 version-7 baseline. AMD-0023 packages the already-ratified held contract phase because the frozen accept replay guard prevents reusing a merged amendment id for a disjoint artifact in a new PR.
- Independent B1 behavior: FS-193's all-degenerate protection is not gated by the contract phase. When a finding generation has quarantined rows and would publish zero rows, it fails before atomic publication with code-owned quarantine count/reason diagnostics and retains the previous accepted generation.
...
### AMD-0023 — Phase 2 of ratified AMD-0021 — shared contract v8 accept

- Status: approved
- Approved packaging: coordinator ruling in FS-193 comment `01M005A04GXJEHVHDP4H4M7PP2`, under amendment-packaging discretion delegated by product owner Matt Wyckhouse; zero new substance beyond the owner's 2026-08-14 08:28 ET ratification of AMD-0021.
- Artifacts:
  - `plugins/bb-plugin-finite-state/shared/contract.ts`
- Contract version: 8
- Prior artifact hash: `b00370339d16e391b9efc4a15246939e63ea2053ef6782699f768e7e812757fd`
- New artifact hash: `041dc7dec66ac92d33b48e947412a2eedff4e7e6bdb54122f5623c9aebd0c466`
- Phase relationship: AMD-0021 is the merged schema-accounting phase; AMD-0023 is only its contract phase. This sibling exists because the accept guard forbids reusing AMD-0021 after its schema target merged.
- Ratified contract scope (verbatim from AMD-0021): add required `quarantined: z.number().int().nonnegative()` to every per-kind value in `pullReportSchema`. The field is the per-generation total across every invocation that contributed to the staged generation, not merely the last invocation's local count. It counts candidate rows for that kind that were individually isolated instead of entering staging; kinds with no isolation events report `0`. `vexDecision` already isolates decided rows through `VEX_REMOTE_IDENTITY_MISSING` advisories, so its `quarantined` value is the sum of those advisory counts. `PullReport.advisories` remains the code breakdown of those same events and must not be added to `quarantined` a second time. Undecided Platform findings that intentionally project to no VEX entity are not VEX quarantine events. Cache pullers and adapters must supply the explicit value rather than relying on omission or a UI default. Preserve the existing meanings of `fetched` and `baseRows`; no finding stable-key or SPEC 02 §4.3 semantics change.
- Ratified migration scope (verbatim from AMD-0021): After the B1 delta is accepted, increment the shared contract from version 7 to 8 and plumb the same generation total through `CachePuller`, the sync engine `PullReport`, `registerFindings`, registered RPC parsing, CLI `--json`, and the Sync panel pull report. Add registered tests asserting mixed finding corpora publish keyable rows with truthful `fetched`, `baseRows`, and `quarantined` counts at RPC, CLI, and panel surfaces; assert non-quarantining kinds report zero.
...
### AMD-0024 — Report isolated per-kind pull outcomes

- Status: approved
- Approved: 2026-08-14 20:15Z, product owner Matt Wyckhouse, verbatim decision `Approved`, relayed by coordinator thread `thr_hg37weivk7` in FS-196 comment `01M00YQ1C8196WFGTS91YZZ62B`.
- Artifacts:
  - `plugins/bb-plugin-finite-state/shared/contract.ts`
- Contract version: 10
- Prior artifact hash: `2b4ecfdfc335188f3165b9acee680da755731cd6661f33f7c2678f6b84621403` (mechanically verified against the AMD-0022 frozen baseline and integration before this change). This is the corrected hash recorded in FS-196 comment `01M00NAZK90EYEFR4E26YGCN6R`; it supersedes stale hash text in the ratification relay without changing the ratified schema or semantics.
- New artifact hash: `a8ddb7d20a3b989f80841b3688781f4e5ee82e5386a37cec668253d1890df30a`
- Reason: contract v9 required one global successful `generationId` and `acceptedAt` and exposed success-only per-kind counts. It therefore could not truthfully represent independent kind generations, a pre-generation Assurance Studio selection refusal, remote failure, or total quarantine while allowing successful sibling kinds to publish.
- Ratified contract: replace the global generation fields with one required discriminated `published | failed` outcome per requested kind. Every outcome carries `fetched`, `baseRows`, `quarantined`, and at most 100 stable reason-code counts. A published outcome requires its own non-null `generationId` and `acceptedAt`; a failed outcome requires at least one reason, always has `acceptedAt: null`, and may have `generationId: null` only when failure precedes generation creation. Partial quarantine remains published with `quarantined > 0`; total quarantine is failed with `FINDING_ALL_ROWS_QUARANTINED` and the complete quarantined count. Producers enforce exact requested-kind key coverage because record validation cannot compare keys to the request.
- Binding base-state condition: after partial publication, `baseStateSha256` is the composite post-publication base state: it incorporates the newly accepted generation and revision for every published kind and the prior accepted generation and revision for every failed kind. It never hashes a failed kind's staging generation and never describes a pre-pull or all-new/all-old synthetic state.
- Failure taxonomy: remote failures consume FS-204's structured `REMOTE_FAILURE_KINDS` through `diagnoseRemoteFailure(...).kind`; contract-bound reason entries do not expose raw request text, credentials, or URLs. `AS_PROJECT_SELECTION_REQUIRED`, quarantine codes, and other code-owned failures remain stable reason identifiers.
- Migration: increment `CONTRACT_VERSION` from 9 to 10; retain the existing store schema and per-kind atomic publication machinery; orchestrate default pulls as independent single-kind transactions; compute the composite base-state hash only after all requested kinds have reached a terminal published/failed outcome. Broadcast the new shape to the registered RPC, CLI, Sync panel, Findings/BOM consumers, Golden Loop, and contract tests. Re-pin the single frozen artifact under AMD-0024 in this same amendment-tagged change.
- Atomicity and emission: one kind either publishes its complete accepted generation or reports failure. A failed kind never advances its accepted pointer. Kind-specific invalidation emits only after that kind's publication commits, so failed kinds emit nothing and cannot suppress successful sibling emissions.
- Affected WPs and gates: FS-196; FS-155; FS-176; FS-179; C-SYNC-TRANSACTION; registered sync RPC and CLI; Sync Review, Findings, and BOM panels; Golden Loop journey; shared contract tests; frozen-artifact guard; Node 22.19 typecheck/test/lint/build gates.
- Approval provenance: owner approval covers the exact FS-196 17:32Z draft schema and semantics, with the supervisor's binding composite `baseStateSha256` condition above. Coordinator verified the draft as well-formed and relayed both approval and condition.
...
- Proposal digest: `6ac18208f38ef1c1f73bcd47e033d03a38879f441c46b8ac9fbb2e80366aa72b`, the SHA-256 of this canonical payload including its final newline:
      ...
      AMD-0025
      base_sha256=a8ddb7d20a3b989f80841b3688781f4e5ee82e5386a37cec668253d1890df30a
      contract_version=11
