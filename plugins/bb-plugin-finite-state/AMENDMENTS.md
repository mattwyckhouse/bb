...
- Artifacts:
  - `plugins/bb-plugin-finite-state/shared/contract.ts`
- Contract version: 8
- Cross-PR dependency: AMD-0020 is not present on `origin/finite-state/integration` as of 2026-08-14; its proposal exists on FS-191 PR #134 at head `b0b16b988`. AMD-0020 must be approved, implemented, and merged first because it owns contract version 7 and adds the required `workspaceProjectId` field to `syncPull` input in this same frozen artifact. AMD-0021 must rebase onto that merged head and must not reuse or bypass version 7.
