...
### FS-208 — stale failed generation resume / SQLite UNIQUE leak (sweep FS-206)
...
- **Journey**: Product Security TARA canvas first load → inspector Fit (empty selection and with selection) → select a node and confirm selection ring; Requirements list against a mixed valid/invalid accepted cache; `bb finite-state as-projects --project <Platform id>` on a remote 401; Verifications / cross-link unreadiness copy
- **Broke because**: first canvas paint clipped under the inspector and Fit only worked for a non-empty selection; invalid `base_snapshot` requirement rows vanished with no banner; CLI RemoteError catch printed a message without code/service/status and `--project` looked like a bb project id; leftover WP-39 jargon reached users after the matrix shipped
- **Beat asserts**: auto-fit runs on first graph identity; inspector Fit fits the full graph when nothing is selected; selected nodes use a primary ring; `requirementsList` cache.message reports invalid cached documents while keeping valid cards; CLI stderr includes `code=`/`service=`/`status=` and usage names Platform project ids; verification unreadiness copy is user-neutral (no WP numbers)

### FS-143 — manual triage follow-ups from FS-40 review (F4, F7, minors)

- **Source**: FS-143 / FS-40 PR #75 review (F4 visible undo, F7 bulk evidence, minors)
