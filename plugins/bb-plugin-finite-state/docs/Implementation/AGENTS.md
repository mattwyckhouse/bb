...
- The **already-built** bidirectional AS push (`lanes/sync/push/*`, frozen human-only `syncPush`/`syncPushRetry`, Sync Review wiring) is **KEPT-FROZEN** (owner refinement 2026-08-14, canonical in AUTHORITY): bugfix yes, new investment no, **do not remove**. It is a human-gated export convenience, never what makes work real.
