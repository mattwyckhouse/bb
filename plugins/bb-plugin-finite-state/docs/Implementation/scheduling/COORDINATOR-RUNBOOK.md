...
Before posting the final verdict, stop your disposable dev instance AND close the browser tabs pointed at it: run `scripts/bb-dev-app stop` (and stop any mock remote you started), then close your `agent-browser` session (`agent-browser session close <session>` or close its tabs). Orphaned dev stacks — server, vite, watchers — keep running from deleted inodes after the worktree is GC'd (~1 GiB each; three zombies fed the 2026-08-14 memory exhaustion). Orphaned browser tabs are just as bad: an SPA tab whose dev instance died spins websocket-reconnect and polling loops against the dead backend indefinitely — dozens of these degraded the whole machine on 2026-08-14 (Chrome at 41 processes / 7.7 GiB). The resource watchdog auto-reaps sessions whose worktree is gone, but it is a 6-hour backstop, not the primary mechanism.
