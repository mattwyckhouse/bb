...
| ----------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Same-field plan conflict / `PLAN_CONFLICT_UNRESOLVED`                                                 | Sync status and plan name the stale tuple; preserve `.fs-sync/plan-<id>.json` and the accepted base.                | Pull status, review the conflict, re-plan, then obtain a new human approval. Never reuse the stale plan hash.                                                    |
...
| Interrupted writer / partial `triage_runs` record or `OVERLAY_CAS_CONFLICT`                           | Authored YAML stays parseable and the durable `triage_runs` row records `status=partial` plus written/error counts. | Inspect the YAML and run row, refresh the preview/CAS fence, and rerun. Convergence means already-written decisions are skipped, not rewritten.                  |
...
| Signature/digest binding invalid / `unverified`, `invalid_signature`, or `stale_digest`               | Preserve the invalid attestation and verdict input; verdict is NOT_SAFE or INCONCLUSIVE, never green.               | Verify current mounted digest, rerun the required check if bytes changed, then sign a new attestation bound to that digest. Do not replace the invalid evidence. |
