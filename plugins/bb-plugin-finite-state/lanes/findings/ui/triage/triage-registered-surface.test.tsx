...
beforeAll(() => {
  ...
  vi.stubGlobal("crypto", { randomUUID: () => "00000000-0000-4000-8000-000000000168" });
...
async function registeredFixture() {
  ...
  for (const [findingId, cve] of [["exact-row-a", "CVE-2026-0168"], ["exact-row-b", "CVE-2026-0169"]] as const) {
    const stableKey = findingStableKey({ cve, purl: "pkg:generic/gateway@1", name: "gateway", version: "1" }, "purl");
    insert.run(findingId, stableKey, cve, "gateway", "1", "pkg:generic/gateway@1", '[{"label":"Call graph","value":"no path","source":"analysis"}]', "2026-08-13T00:00:00.000Z");
  }
  ...
  it("writes YAML when the registered Findings panel submits the registered bulk RPC", async () => {
    ...
    const slot = renderSlot(panel, { subPath: "" }, {
      ...
      rpc: {
        ...
        cachedProjectVersions: async input => findingsUiRpcContract.cachedProjectVersions.output.parse(await host.harness.callRpc("cachedProjectVersions", input)),
        findingsSavedViewsGet: () => ({ views: [], sha256: null, recoveredFromCorrupt: false }),
        findingsUiList: async input => findingsUiRpcContract.findingsUiList.output.parse(await host.harness.callRpc("findingsUiList", input)),
        triageTargetsRead: async input => findingsUiRpcContract.triageTargetsRead.output.parse(await host.harness.callRpc("triageTargetsRead", input)),
        triageDecisionsWrite: async input => {
          const result = findingsUiRpcContract.triageDecisionsWrite.output.parse(await host.harness.callRpc("triageDecisionsWrite", input));
    ...
    await waitFor(() => expect(slot.container.querySelectorAll("[data-finding-row]")).toHaveLength(2));
    fireEvent.click(slot.getByRole("button", { name: "Select all 2" }));
    fireEvent.keyDown(window, { key: "b" });
    fireEvent.click(slot.getByRole("button", { name: /eEXPLOITABLE/u }));
    const editor = await slot.findByRole("form", { name: /2 local overlay identities/u });
