...
import type { VexJustification, VexResponse } from "../../../../lib/remote/types.js";
      ...
      <div className="flex items-start gap-4 px-4 py-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Local YAML decision</p>
              <h2 className="mt-0.5 truncate text-sm font-semibold">{targetLabel}</h2>
              <p className="mt-1 font-mono text-xs text-primary">{draft.status}</p>
            </div>
            <div className="flex items-center gap-2">
              <Button onClick={onCancel} size="sm" type="button" variant="ghost">Cancel</Button>
              <Button aria-keyshortcuts="Control+Enter Meta+Enter" disabled={pending || !reasonConfirmed || !validation.ok} size="sm" type="submit">
                {pending ? <Icon aria-hidden="true" className="size-4 animate-spin" name="Loading" /> : <Icon aria-hidden="true" className="size-4" name="CircleCheck" />}
                Write YAML <kbd className="ml-1 text-[0.7rem] opacity-70">⌘↵</kbd>
              </Button>
            </div>
          </div>
          ...
          <div className="mt-4 grid gap-4 xl:grid-cols-2">
            {draft.status === "NOT_AFFECTED" ? <JustificationPicker onChange={justification} value={draft.justification} /> : (
              <div className="space-y-2">
                ...
                <p className="rounded-md border border-border bg-muted/30 px-3 py-2 text-xs text-muted-foreground">Not required for {draft.status.replaceAll("_", " ")}.</p>
              </div>
            )}
            <div className="space-y-2">
              <Label id="triage-response-label">Response <span className="font-normal text-muted-foreground">optional</span></Label>
              <Select onValueChange={value => onChange({ ...draft, response: value === "none" ? null : value as VexResponse })} value={draft.response ?? "none"}>
                <SelectTrigger aria-label="Response" aria-labelledby="triage-response-label" id="triage-response"><SelectValue /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="none">No response</SelectItem>
                  {VEX_RESPONSE_VALUES.map(response => <SelectItem key={response} value={response}>{response.replaceAll("_", " ")}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-2 xl:col-span-2">
              <Label htmlFor="triage-reason">Reason</Label>
              <Textarea id="triage-reason" onChange={event => { onReasonConfirmed(false); onChange({ ...draft, reason: event.target.value }); }} rows={3} value={draft.reason} />
              {seededReason ? <p className="text-xs text-warning">Seeded from cached reachability evidence. Review and explicitly confirm it; a seed is not approval.</p> : null}
            </div>
            <div className="space-y-2 xl:col-span-2">
              <Label htmlFor="triage-evidence">Evidence reviewed</Label>
              <Textarea id="triage-evidence" onChange={event => { onReasonConfirmed(false); onChange({ ...draft, evidence: event.target.value }); }} rows={3} value={draft.evidence} />
            </div>
            <div className="space-y-2">
              <Label id="triage-pin-label">Version pin</Label>
              <Select disabled={exactPin} onValueChange={value => onChange({ ...draft, pin: value as TriageDraft["pin"] })} value={draft.pin}>
                <SelectTrigger aria-describedby={exactPin ? "triage-pin-locked" : undefined} aria-label="Version pin" aria-labelledby="triage-pin-label" id="triage-pin"><SelectValue /></SelectTrigger>
                <SelectContent><SelectItem value="exact_version">Exact version</SelectItem><SelectItem value="any_version">Any version</SelectItem></SelectContent>
              </Select>
              {exactPin ? <p className="text-xs text-muted-foreground" id="triage-pin-locked">CODE_NOT_REACHABLE evidence is build-specific, so promotion is disabled.</p> : null}
            </div>
            <label className="flex items-start gap-2 self-end rounded-md border border-border bg-muted/20 p-3 text-xs">
              <Checkbox checked={reasonConfirmed} onCheckedChange={checked => onReasonConfirmed(checked === true)} />
              <span><strong className="block font-medium text-foreground">I reviewed this reason and evidence</strong><span className="mt-1 block text-muted-foreground">The plugin will record exactly this authored rationale.</span></span>
            </label>
          </div>
          ...
          {error ? (
            <div className="mt-3 rounded-md border border-destructive/40 bg-muted p-3 text-xs" role="alert">
              ...
              {error.kind === "conflict" ? <div className="mt-3 flex gap-2"><Button onClick={onReload} size="sm" type="button" variant="outline">Reload CAS base</Button><Button onClick={() => setShowCompare(value => !value)} size="sm" type="button" variant="ghost">Compare</Button></div> : null}
              {showCompare ? <p className="mt-2 rounded border border-border bg-background p-2 text-muted-foreground">Your draft remains above. Compare it with {error.file ?? "the newer overlay"} in the Changes panel, then reload the CAS base before retrying.</p> : null}
            </div>
          ) : null}
        </div>
      </div>
