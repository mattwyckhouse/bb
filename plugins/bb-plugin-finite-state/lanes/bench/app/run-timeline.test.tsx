...
class BenchResizeObserver implements ResizeObserver {
  ...
  observe(target: Element): void { queueMicrotask(() => this.callback([{ target, contentRect: new DOMRectReadOnly(0, 0, 900, 600), borderBoxSize: [{ blockSize: 600, inlineSize: 900 }], contentBoxSize: [{ blockSize: 600, inlineSize: 900 }], devicePixelContentBoxSize: [{ blockSize: 600, inlineSize: 900 }] } as ResizeObserverEntry], this)); }
  unobserve(): void {}
  disconnect(): void {}
}
...
beforeAll(() => {
  ...
  HTMLElement.prototype.scrollTo = function scrollTo(options?: ScrollToOptions | number, y?: number) { this.scrollTop = typeof options === "number" ? (y ?? 0) : (options?.top ?? 0); this.dispatchEvent(new Event("scroll")); };
});
afterEach(() => cleanup());

function run(index: number) {
  return { projectId: "p1", projectVersionId: "v1", kind: "verificationRun", key: `run-${index}`, label: "run", fields: { tier: index % 2 ? "tier1" : "tier0", kind: "verification", trigger: index % 2 ? "manual" : "sync", status: index === 0 ? "failed" : "completed", firmwareDigest: "a".repeat(64), threadId: `thread-${index}`, startedAt: "2026-08-13T12:00:00.000Z", finishedAt: "2026-08-13T12:00:01.000Z", signed: index !== 0 } };
}
...
describe("RunTimeline", () => {
  ...
  it("virtualizes, filters, pages stably, links the native thread, and refetches late status", async () => {
    ...
    expect(slot.container.querySelectorAll("[data-bench-run-row]").length).toBeLessThan(50);
    expect(slot.getByText("Unsigned")).toBeTruthy();
    fireEvent.change(slot.getByLabelText("Filter by tier"), { target: { value: "tier0" } });
    fireEvent.click(slot.getByLabelText("Failing only"));
    expect(slot.getByText("run-0")).toBeTruthy();
    fireEvent.click(slot.getByRole("button", { name: "Open native thread for run-0" }));
    expect(slot.inspection.navigateCalls).toContainEqual({ method: "toThread", threadId: "thread-0" });
    fireEvent.click(slot.getByRole("button", { name: "Load older runs" }));
    await waitFor(() => expect(slot.inspection.rpcCalls.some((call) => Reflect.get(call.input as object, "continuation") === "older")).toBe(true));
    await slot.behavior.emitRealtime("bench:changed", { runId: "run-0", status: "completed" });
