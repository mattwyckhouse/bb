...
export function BenchPanel({
  ...
  const [launcherTier, setLauncherTier] = useState<"tier0" | "tier1" | null>(
    null,
  );
  const projectId = routeProjectId ?? selectedProjectId;
  const platformProjectId =
    selectedVersion?.platformProjectId ?? projectId ?? "";
  const projectVersionId = selectedVersion?.projectVersionId ?? "";
  ...
  const runDisabledReason = !projectId
    ...
    : versionsLoading
      ...
      : versionsError
        ? `Cached bench versions could not be loaded: ${versionsError}`
        : !projectVersionId
          ? versions.length === 0
  ...
  useEffect(() => {
    ...
    void Promise.resolve()
      .then(() => {
        ...
        return rpc.call("benchProjectVersions", { projectId });
      })
      .then((result) => {
        if (!active) return;
      ...
      .catch((cause: unknown) => {
        ...
        setVersionsError(
          cause instanceof Error
            ? cause.message
            : "Cached versions could not be loaded.",
        );
      })
      .finally(() => {
  ...
  const header = (
    <header className="flex h-12 shrink-0 items-center gap-2 border-b border-border bg-card px-3">
      ...
      <label
        ...
        htmlFor="bench-project-version"
      >
        Version
      </label>
      <select
        aria-label="Bench project version"
        className="h-8 max-w-64 rounded-md border border-input bg-background px-2 font-mono text-xs"
        disabled={!projectId || versionsLoading}
        id="bench-project-version"
        onChange={(event) => {
          clearRunRoute();
          setSelectedVersion(
            versions.find(
              (version) => version.projectVersionId === event.target.value,
            ) ?? null,
          );
        }}
      ...
      <Button
        aria-describedby={
          runDisabledReason ? "bench-header-run-reason" : undefined
        }
        disabled={runDisabledReason !== null}
        onClick={() => setLauncherTier("tier0")}
        size="sm"
      >
        <Icon name="Workflow" />
        Run
        ...
        <div className="flex min-h-0 flex-1 items-center justify-center p-6">
          <div className="max-w-md rounded-lg border border-border bg-card p-6 text-center">
            ...
            <p className="mt-2 text-sm leading-6 text-muted-foreground">
              The bench timeline is scoped to a bb project. Choose one above; an
              explicit Finite State version narrows the history and is required
              to launch.
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-1 items-center justify-center p-6">
          <div className="max-w-md rounded-lg border border-border bg-card p-6 text-center">
            ...
            <h2 className="mt-3 text-lg font-semibold">
              Invalid bench run route
            </h2>
            <p className="mt-2 text-sm text-muted-foreground">
              No request was sent for this unbounded run identifier.
            </p>
            <Button
              className="mt-4"
              onClick={() =>
                navigate.toPluginPanel("bench", { subPath: "", replace: true })
              }
              variant="outline"
            >
              Return to timeline
  ...
  if (launcherTier) {
    return (
      <section className="flex h-full min-h-0 flex-col">
        {header}
        {projectVersionId.trim() ? (
          <div className="min-h-0 flex-1">
            <RunLauncher
              initialTier={launcherTier}
              onClose={() => setLauncherTier(null)}
              onFailed={(failedRunId) => {
                setLauncherTier(null);
                navigate.toPluginPanel("bench", {
                  subPath: encodeURIComponent(failedRunId),
                });
              }}
              onStarted={(startedRunId) =>
                navigate.toPluginPanel("bench", {
                  subPath: encodeURIComponent(startedRunId),
                })
              }
  ...
  return (
    <section className="flex h-full min-h-0 flex-col bg-background text-foreground">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-12">
        <div
          className={`${runId ? "col-span-5" : "col-span-7"} min-h-0 border-r border-border`}
        >
          <RunTimeline
            onOpen={openRun}
            onRunTier0={() => setLauncherTier("tier0")}
            projectId={platformProjectId}
            projectVersionId={projectVersionId.trim() || null}
        ...
        {runId ? (
          ...
          <aside
            aria-label="Bench verdict summary"
            className="col-span-5 min-h-0 overflow-auto p-4"
          >
            <VerdictCard
              id={projectVersionId || undefined}
              projectId={platformProjectId}
            />
          </aside>
        )}
